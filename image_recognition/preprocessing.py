...
# Mean is an array of three elements obtained by the average of 
# R, G, B pixels of all images obtained from ImageNet. 
...
class ImagePreprocessor(object):
    ...
    def preprocess(self, image_buffer, bbox, thread_id):
        ...
        # Normalize the image with ImageNet RGB channel means
        imagenet_mean_tensor = tf.constant(imagenet_mean, dtype=tf.float32)
        imagenet_mean_tensor = tf.reshape(imagenet_mean_tensor, [1, 1, 3])
        image = tf.subtract(image, imagenet_mean_tensor)
    ...
    def minibatch(self, dataset, subset):
        with tf.name_scope('batch_processing'):
            ...
            for i in xrange(self.batch_size):
                ...
                image = self.preprocess(image_buffer, None, i % 4)                
                images.append(image)
            ...
            depth = 3
            images = tf.reshape(images, shape=[self.batch_size_per_device, self.height, self.width, depth])
            labels = tf.reshape(labels, [self.batch_size_per_device])
            filenames = tf.reshape(filenames, [self.batch_size_per_device])

            return images, labels, filenames, records
