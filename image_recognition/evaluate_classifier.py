...
class eval_classifier_graph:
    """Evaluate image classifier with TensorFlow graph"""
    ...
    def __init__(self):
        ...
        arg_parser.add_argument("--input-layer", default="input",
                                help="name of input layer")
        arg_parser.add_argument("--output-layer", default="predict",
                                help="name of output layer")
        arg_parser.add_argument("--image-width", type=int, default=224, 
                                help="The input image width for the network")
        arg_parser.add_argument("--image-height", type=int, default=224, 
                                help="The input image height for the network")
        arg_parser.add_argument("--labels-offset", type=int, default=0, 
                                help="labels_offset is 1 if inference model output 1000 classes without background")
    ...
    def run(self):
        ...
        with data_graph.as_default():
            if (self.args.data_location):
                ...
                preprocessor = dataset.get_image_preprocessor()(
                    self.args.image_height, self.args.image_width, self.args.batch_size,
                    resize_method='bilinear')

                images, labels, filenames, records = preprocessor.minibatch(dataset, subset=subset)
                ...
                input_shape = [self.args.batch_size, self.args.image_height, self.args.image_width, 3]
        ...
        input_tensor = infer_graph.get_tensor_by_name(self.args.input_layer + ":0")
        output_tensor = infer_graph.get_tensor_by_name(self.args.output_layer + ":0")
        ...
        if (not self.args.accuracy_only):
            ...
            while num_remaining_images >= self.args.batch_size:
                ...
                # VGG and RESNET don't have background class
                np_labels = np_labels - self.args.labels_offset
...
if __name__ == "__main__":
    evaluate_graph = eval_classifier_graph()
    evaluate_graph.run()
