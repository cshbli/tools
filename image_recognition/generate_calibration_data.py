...
if __name__ == "__main__":
    ...
    parser.add_argument("--labels_offset", type=int, default=0, 
                        help="labels offset if inference model output 1000 classes without background")
    parser.add_argument("--output_calibration_filepath", type=str,
                        default="./calibration-1-of-1", help="The filepath of output calibration dataset TFRecord")
    parser.add_argument(
        '--num_inter_threads',
    ...
    images, labels, filenames, tf_records = preprocessor.minibatch(dataset, subset='validation')    
    ...
    with tf.Session() as sess:
        ...
        while num_remaining_images >= batch_size:
            ...
            np_images, np_labels, np_filenames, serialized_images = sess.run(
                [images, labels, filenames, tf_records])
            num_processed_images += batch_size
            num_remaining_images -= batch_size
            
            # HACK for TESTING the image extraction is good
            
            # int_images = sess.run(tf.cast(images, tf.uint8))            
            # import matplotlib.image as mpimg
            # mpimg.imsave("temp.jpg", np_images[0])
            # print(np_labels[0])
            # break            

            np_labels = np_labels - args.labels_offset
    ...
    writer = tf.python_io.TFRecordWriter(args.output_calibration_filepath)
